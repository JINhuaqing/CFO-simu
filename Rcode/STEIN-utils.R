...
get.oc <- function(target, pE.true,pT.true,  psi1, psi2, ncohort, cohortsize, startdose=1, cutoff.eli=0.95, ntrial=10)
{
	...
	for(trial in 1:ntrial)
	{
		...
		{  			
			...
			if(n[d]>=3 && pbeta(0.2,yE[d]+1,n[d]-yE[d]+1)>0.95) {
			    elimiE[d]=1;
			}
			...
			if (yT[d]>=b.d[nc] && d!=1) {
			    ...
			    d_opt=d
			       ...
			       if (yE[d]/n[d]>psi) {
		    	       ...
		    	       if (elimi[d+1]==0){
		    	           posH<-c(pbeta(psi,yE[d-1]+1,n[d-1]-yE[d-1]+1),pbeta(psi,yE[d]+1,n[d]-yE[d]+1), (n[d+1]>0)*pbeta(psi,yE[d+1]+1,n[d+1]-yE[d+1]+1))
		    	                 -c(0,0.001,0.002)
				           d_opt=d-2+which.min(posH)		    	       	
		    	       } else {
			               posH<-c(pbeta(psi,yE[d-1]+1,n[d-1]-yE[d-1]+1), pbeta(psi,yE[d]+1,n[d]-yE[d]+1))-c(0,0.001)
...
get.oc.raw <- function(target, pE.true,pT.true,  psi1, psi2, ncohort, cohortsize, startdose=1, cutoff.eli=0.95, ntrial=10)
{
	peestimate<-function(yE,n){
	    ndose<-length(yE)
	    lik<-rep(0,ndose)
			...
			if (yT[d]>=b.d[nc] && d!=1) {
			    ...
			    d_opt=d
			       ...
			       if (yE[d]/n[d]>psi) {
			           ...
			           posH<-c(pbeta(psi,yE[max(1,d-1)]+1,n[max(1,d-1)]-yE[max(1,d-1)]+1), pbeta(psi,yE[d]+1,n[d]-yE[d]+1))
                       d_opt<-max(d-2+which.min(posH),1);
			       } else if (yT[d]<=b.e[nc] && d==1) {
			           if (elimi[d+1]==0) {
			               posH<-c(pbeta(psi,yE[d]+1,n[d]-yE[d]+1), (n[d+1]>0)*pbeta(psi,yE[d+1]+1,n[d+1]-yE[d+1]+1))
			               d_opt=d-1+which.min(posH)
			           } else {
			               d_opt=d
			           }
