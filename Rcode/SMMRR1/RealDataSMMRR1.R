...
ORM.Eff.simu.fn.realdata <- function(phi, phiE, p.true, pE.true, ncohort=10, init.level=1,  cohortsize=3, add.args=list(),
    ...
    for (i in 1:ncohort){
        ...
        res.all <- make.decision.ORM.fn(phi, cys, cns, add.args$alp.prior, add.args$bet.prior, cover.doses, diag=TRUE)
        odds.res[[i]] <- res.all
        up.idx <- res.all$cidx - 2 + cidx
...
# plot the procedure 
effs.Vec <- as.vector(t(res$eff.seq))
toxs.Vec <- as.vector(t(res$tox.seq))
chts.x <- rep((1:19)*2, each=3) + c(-0.5, 0, 0.5)
dose.y.tox <- rep(res$dose.seq, each=3) + 0.1
