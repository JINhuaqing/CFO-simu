...
source("./SMMRR1/anova_settings.R")
source("ORM_utils.R")
        ...
        for (i3 in 1:length(sample.sizes)){ # sample size
            for (i4 in 1:length(diff.probs)){
                for (i5 in 1:length(targets)){
                    ...
                    # get model skeleton of CRM
                    ncohort.fake <- sample.size/3
                    crm.fName <- paste0("../results/SMMR-R1/CRM_cal_phi", target*100, "_ndose_", nlevel, "_ncohort_", ncohort.fake, ".RData")
                   load(crm.fName)
                   deltas <- seq(0.01, min(target-0.005, 0.15), 0.01)
                   hfInter <- deltas[which.max(res)]
                   p.prior <- getprior(hfInter, target, ceiling(nlevel/2), nlevel)
                    ...
                    run.fn <- function(k){
                        ...
                        crm.res <- crm.simu.fn(target=target, p.true=p.true, cohortsize=cohortsize, ncohort=ncohort, p.prior=p.prior)
                    ...
                    file.name <- paste0("../results/", "SMMR-R1/ANOVA/ANOVA", i1, i2, i3, i4, i5, "_", nsimu, ".RData")
