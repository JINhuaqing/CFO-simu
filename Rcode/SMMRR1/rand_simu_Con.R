...
run.fn <- function(k){
    ...
    ormcon.res <- ORMcon.simu.fn(target, p.true, ncohort=ncohort, cohortsize=cohortsize, add.args=add.args)
...
nsimu <- 5000
seeds <- 1:nsimu
file.name <- paste0("../results/SMMR-R1/", "ConWay2_MTDSimu_", nsimu, "random_0.05",  ".RData")
