...
make.decision.ORMcon.fn <- function(phi, cys, cns, alp.prior, bet.prior, cover.doses, diag=FALSE){
    if (cover.doses[2] == 1){
        return(1)
    }else{
...
ORMcon.simu.fn <- function(phi, p.true, ncohort=12, init.level=1, 
    ...
    for (i in 1:ncohort){
        ...
        idx.chg <- make.decision.ORMcon.fn(phi, cys, cns, add.args$alp.prior, add.args$bet.prior, cover.doses) - 2
        if (idx.chg == -2){
            contradic.count[i] <- 1
            idx.chg <- -1
        }
