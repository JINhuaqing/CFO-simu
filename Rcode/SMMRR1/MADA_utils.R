...
MADA.simu.fn <- function(phi, phiE, p.true, pE.true, ncohort=10, init.level=1,  cohortsize=3){
    ...
    total.s1=ceiling(0.4*ncohort)*csize; # sample size in stage 1
    total.ss=ncohort*csize; # sample size in both stage 2
    SKL=rep(0,length(pE.true));
    
    skeleton=matrix(c(prob1,prob2,prob3,prob4,prob5,prob6),length(prob1),length(prob1))
    MTD=rep(0, length(p.true))
    ...
    prior_mean<-seq(0.1,0.1*length(p.true),0.1);
    ...
    dose.curr=init.level;##start from the first dose level
    dose=NULL;
    y=NULL;
    ...
    while ((sum(dose.npat)<total.s1)&&(stop1==0)){
      ...
      eff.curr=rbinom(csize, 1, pE.true[dose.curr]);
      dose.eff[dose.curr]=dose.eff[dose.curr]+sum(eff.curr);

      tox.curr=rbinom(csize, 1, p.true[dose.curr]);
      ...
      pp.np=1-pbeta(phi,alpha0[dose.curr]+dose.tox[dose.curr],beta0[dose.curr]+dose.npat[dose.curr]-dose.tox[dose.curr])
      ...
      pp.p=integrate(function(tau)
       	             {
          	             sapply(tau,function(tau)
            	                 {
            	                   integrate(function(mu) posterior1(mu,tau,dose.tox,dose.npat,dose.level), -Inf, dose.level[j]-sqrt(tau)*qnorm(phi))$value
            	                 })
    ...
    pi_hat=rep(0,length(p.true));
    
    for(j in 1:length(p.true)) { 
    ...
    dose.star=max(which(abs(pi_hat-phi)==min(abs(pi_hat-phi))));
    ...
    dose.star=max(which.min(abs(dose.pava-phi)));
    MTD[dose.star]=MTD[dose.star]+1;
    ...
    eff.ns <- dose.eff;
    dose.ns <- dose.npat;
    res <- list(OBD=OBD, dose.ns=dose.ns, eff.ns=eff.ns, DLT.ns=DLT.ns, pE.true=pE.true, min.eff=phiE, p.true=p.true, target=phi)
    res
}
