...
source("./Rcode/SMMRR1/anova_settings.R")
anova.ress <- dir("./results/SMMR-R1/ANOVA", pattern="*.RData", full.names = TRUE)

sep.paths <- strsplit(anova.ress, "_")
sep.paths <- sapply(sep.paths, function(i)i[1])
sep.paths2 <- strsplit(sep.paths, "A")
raw.labs <- sapply(sep.paths2, function(i)i[5])
tmp.fn <- function(i){
    if (length(i)==5){
        return(as.numeric(i))
    }else{
