...
run.fn2 <- function(k){
    print(k)
    res <- ORM.Eff.simu1.fn(phi, phiE, p.true=p.true, pE.true=pE.true, add.args=add.args, cohortsize=cohortsize, ncohort=ncohort, ph1=0)
    res
}

ress <- mclapply(1:nsimu, run.fn1, mc.cores=8)
